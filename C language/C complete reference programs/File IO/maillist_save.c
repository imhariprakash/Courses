...
void save_file(void)
{
    int i=0;
    FILE *file;
    if((file=fopen("maillist_save.txt","wb"))==NULL)
    {
        printf("Can't able to open the file for writing.\n");
        exit(1);
    }

    while(add_list[i].name[0] != '\0')
    {
        if((fwrite(&add_list[i],sizeof(add_list[i]),1,file))!=1)
        {
...
void load(void)
{
    ...
    if((file=fopen("maillist_save.txt","rb"))==NULL)
    {
